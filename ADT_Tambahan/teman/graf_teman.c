...
void DisplayFriendRequests(GrafTeman* G, Pengguna user) {
    ...
    if (0 == NEffRequests) {
        ...
        printf("Terdapat %d permintaan pertemanan untuk Anda.", NEffRequests); newline;


        for (i = 0; i < NEffRequests; i++) {

            printf("| "); displayWord(requests[i].pengirim.nama); newline;
            printf("| Jumlah teman: %d", requests[i].BanyaknyaTeman); newline;
        }
    }
}
        ...
        boolean correct_input = false;
            do
            {
                ...
                if (isSameWord(currentWord, stringToWord("YA", 2))) {
                    ...
                    printf("Permintaan pertemanan dari "); displayWord(requests[0].pengirim.nama);
