...
void DisplayTeman(GrafTeman* G, Pengguna user) {
    ...
    displayWord(user.nama); printf(" memiliki %d teman.", banyak_teman_orang_ini); newline;
    printf("Daftar teman "); displayWord(user.nama); newline;
    
    for (i = 0; i < G->NEffVertex; i++) {
       if ((isTeman(G, G->ListVertex[i], user)) && !isSameWord(user.nama, currentPengguna.nama)) {
            printf("| "); displayWord(G->ListVertex[i].nama); newline;
       }
    }
}
...
void DAFTAR_TEMAN() {
    if (BanyakTeman(&dataTeman, currentPengguna) == 0) {
        displayWord(currentPengguna.nama); printf(" belum mempunyai teman."); newline;
    }
    else {
        DisplayTeman(&dataTeman, currentPengguna); newline;
    }
}
...
void HAPUS_TEMAN() 
{
    ...
    if (BanyakTeman(&dataTeman, currentPengguna) == 0) {
        ...
        if (isSameWord(query_user, currentPengguna.nama)) {
            printf("Anda tidak bisa menghapus diri sendiri... Silakan masukkan pengguna lain."); newline;
        }
            ...
            do
            {
                ...
                if (isSameWord(currentWord, stringToWord("YA", 2))) {
                    ...
                    printf("Input tidak dikenali. Perhatikan lagi instruksi input yang diberikan"); newline; newline;
                }
            ...
            displayWord(query_user); printf(" bukan teman Anda");
            newline;
        }
    }
}
...
void TAMBAH_TEMAN() {
    ...
    if (kasus == 1) {
        ...
        printf("Pengguna bernama "); displayWord(currentWord);
        printf(" tidak ditemukan.");
    }
    else if (kasus == 3) {
        printf("Anda sudah berteman dengan "); displayWord(currentWord);
    }
    else if (kasus == 4) {
        printf("Anda sudah mengirimkan permintaan pertemanan kepada "); 
        displayWord(query_user);
        printf(". Silakan tunggu hingga permintaan Anda disetujui."); newline;
    }
    else if (kasus == 5) {
        printf("Woi ah apesi!\n");
        printf("%d", R.BanyakTeman); newline;
        displayWord(R.sender.nama); newline;
        printf("%d", getIdPengguna(query_user)); newline;
...
void DAFTAR_PERMINTAAN_PERTEMANAN() {
    ...
    if (isEmptyFriendReq(&dataFriendRequest.buffer[id])) {
        ...
        for (i = 0; i < banyak_teman_ini_orang; i++)
        {
            ...
            printf("| "); displayWord(req.sender.nama); newline;
            printf("| Jumlah teman: %d", req.BanyakTeman); newline;
        }
    }
    ...
    if (lengthFriendReq(&dataFriendRequest.buffer[id]) == 0) {
        ...
        do
        {
            ...
            if (isSameWord(currentWord, stringToWord("YA", 2))) {
                ...
                printf("Permintaan pertemanan dari "); displayWord(req.sender.nama);
                printf(" telah ditolak.");
            }

            else {
                printf("Input tidak dikenali."); newline; newline;
            }
